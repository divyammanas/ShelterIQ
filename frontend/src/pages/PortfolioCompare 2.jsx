...
export const PortfolioCompare = () => {
    ...
    const getRankStars = (score) => {
        const active = Math.min(Math.max(Math.ceil((score - 50) / 10), 1), 5);
        const stars = [];
        for (let i = 1; i <= 5; i++) {
            stars.push(<Star key={i} size={14} className={i <= active ? "fill-amber-400 text-amber-400" : "text-zinc-300 dark:text-zinc-700"}/>);
        }
        return stars;
    };
...
<<<<<<< Updated upstream:frontend/src/pages/PortfolioCompare.jsx
        {savedDesigns.map((design, idx) => (<div key={idx} className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm flex flex-col gap-4 relative overflow-hidden">
=======
        {savedDesigns.map((design, idx) => (
          <div 
            key={idx}
            className="bg-white dark:bg-[#121622] border border-slate-200 dark:border-[#1e2638] rounded-xl p-5 shadow-xs flex flex-col gap-4 relative overflow-hidden"
          >
            {/* Rank badge */}
>>>>>>> Stashed changes:frontend/src/pages/PortfolioCompare.tsx
...
=======
        {savedDesigns.length === 0 && (
          <div className="col-span-full bg-white dark:bg-[#121622] border border-slate-200 dark:border-[#1e2638] rounded-xl p-8 shadow-xs text-center text-slate-400 font-mono text-xs">
...
>>>>>>> Stashed changes:frontend/src/pages/PortfolioCompare.tsx
        <div className="flex items-center justify-between p-5 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 hover:bg-zinc-50 dark:hover:bg-zinc-900 text-zinc-700 dark:text-zinc-300 text-xs font-semibold px-4 py-2.5 rounded-xl transition-all duration-150 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-xs text-zinc-600 dark:text-zinc-300">
            ...
            <tbody className="divide-y divide-zinc-200/50 dark:divide-zinc-800/40 font-medium">
              {savedDesigns.map((design, idx) => (<tr key={idx} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors">
